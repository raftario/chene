import { l as l$1, y as y$1, g as g$1 } from './preact-bC3ePeIp.js';
...
class Chain {
    ...
    use(middleware) {
        ...
        return new Chain((input, next) => current(input, (middlewareInput) => m(middlewareInput, next)));
    }
    /**
 ...
 */
function status(status, options) {
    return new Response(statuses$1(status), { ...options, status: status });
}
...
function validate(route) {
    ...
    for (let i = 0; i < route.length; i++) {
        ...
        if (!valid)
            throw new Error(`Invalid character in route (${route[i]})`);
    }
}
...
function update(path, node, f, prefix) {
    ...
    if (c === COLON) {
        ...
        if (node.one && node.one.verbatim !== verbatim) {
            throw new Error(`Cannot assign different names to same :named segment (${node.one.verbatim} vs ${verbatim})`);
        }
        else if (!node.one) {
    ...
    // capture any segments
        ...
        if (node.any && node.any.verbatim !== path) {
            throw new Error(`Cannot assign different names to same *named segment (${node.any.verbatim} vs ${path})`);
        }
        else if (!node.any) {
    ...
    insert(route, value) {
        ...
        update(normalise(route), this.#root, (node) => {
            if (node.value !== undefined)
                throw new Error(`Conflicting route (${route}))`);
            node.value = value;
        }, this.#root.verbatim);
    }
    summary() {
