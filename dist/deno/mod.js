...
import { B as BodyTypeError, a as BodyValidationError, b as BodyError, Q as QueryValidationError, U as UrlError } from './router-QHs8whUM.js';
export { C as Chain, d as asMiddleware, r as response, c as router, t as throwInMiddleware } from './router-QHs8whUM.js';
import './preact-bC3ePeIp.js';
...
function fromString(type) {
    if (type instanceof z.ZodString ||
        ...
        return z.union(type.options.map(fromString));
    }
    else {
        throw new TypeError();
    }
}
function fromStrings(type) {
    if (type instanceof z.ZodArray) {
        return z.array(fromString(type.element)).default([]);
    }
    else if (type instanceof z.ZodTuple) {
        return z.tuple(type.items.map(fromString));
    }
    else if (type instanceof z.ZodOptional) {
        ...
        return z.union(type.options.map(fromStrings));
    }
    else {
        return z.tuple([fromString(type)]).transform(([v]) => v);
    }
}
function fromStringsMap(type) {
    if (type instanceof z.ZodRecord) {
        return z.record(type.keySchema, fromStrings(type.valueSchema));
    }
    else if (type instanceof z.ZodObject) {
        return z.object(Object.fromEntries(Object.entries(type.shape).map(([k, v]) => [k, fromStrings(v)])));
    }
    else if (type instanceof z.ZodEffects) {
        return fromStringsMap(type.innerType()).pipe(type);
    }
    else if (type instanceof z.ZodUnion) {
        return z.union(type.options.map(fromStringsMap));
    }
    else {
        throw new TypeError();
    }
...
function form(...args) {
    const key = typeof args[0] === "string" ? args.shift() : "body";
    const schema = args[0];
    return async (input, next) => {
        const contentType = input.request.headers.get("content-type");
        if (!contentType || parse_1(contentType).type !== CONTENT_TYPE) {
...
function query(...args) {
    const key = typeof args[0] === "string" ? args.shift() : "query";
    const schema = args[0];
    if (schema) {
        return async (input, next) => {
            const map = {};
            for (const [k, v] of input.url.searchParams)
                (map[k] ??= []).push(v);
            const parsed = await fromStringsMap(schema).safeParseAsync(map);
            if (!parsed.success) {
